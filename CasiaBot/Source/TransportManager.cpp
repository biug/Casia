...
void TransportManager::followPerimeter(BWAPI::Position to, BWAPI::Position from)
{
	...
	if (_transportShip->getDistance(from) < 50 && _waypoints.empty())
	{
		...
		CAB_ASSERT_WARNING(valid, "waypoints not valid! (transport mgr)");
...
std::pair<int,int> TransportManager::findSafePath(BWAPI::Position to, BWAPI::Position from)
{
	...
	CAB_ASSERT_WARNING(endPolygonIndex != -1, "Couldn't find a closest vertex");
	...
	CAB_ASSERT_WARNING((_minCorner.isValid() && _maxCorner.isValid()), "Map corners should have been set! (transport mgr)");
...
{
	CAB_ASSERT_WARNING(!_mapEdgeVertices.empty(), "We should have a transport route!");
