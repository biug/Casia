...
void BOSSManager::startNewSearch(const std::vector<MetaPair> & goalUnits)
{
    ...
    // convert from CasiaBot's meta goal type to BOSS ActionType goal
    try
    {
...
void BOSSManager::update(double timeLimit)
{
    ...
    if (isSearchInProgress())
    {
        ...
        {
            CAB_ASSERT_WARNING(false, "BOSS SmartSearch Exception: %s", exception.what());
        ...
        if (previousSearchComplete)
        {
            ...
            if (!solved && _previousBuildOrder.size() == 0)
            {
                ...
                {
                    CAB_ASSERT_WARNING(false, "BOSS Timeout Naive Search Exception: %s", exception.what());
