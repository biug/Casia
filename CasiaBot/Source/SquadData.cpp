...
void SquadData::removeSquad(const std::string & squadName)
{
    ...
    CAB_ASSERT_WARNING(squadPtr != _squads.end(), "Trying to clear a squad that didn't exist: %s", squadName.c_str());
...
{
    if (!Config::Debug::DrawSquadTypeInfo)
    {
        return;
    }
	...
	for (auto & kv : _squads) 
	{
        ...
        {
            BWAPI::Broodwar->drawTextMap(unit->getPosition() + BWAPI::Position(0, -10), "\x04 %s", squad.getName().c_str());
        }
	}
}
...
{
    CAB_ASSERT_WARNING(canAssignUnitToSquad(unit, squad), "We shouldn't be re-assigning this unit!");
...
{
    CAB_ASSERT_WARNING(squadExists(squadName), "Trying to access squad that doesn't exist: %s", squadName);
