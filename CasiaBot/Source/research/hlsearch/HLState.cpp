...
HLState::HLState(BWAPI::GameWrapper & game, BWAPI::PlayerInterface * player, BWAPI::PlayerInterface * enemy)
{
	...
	CAB_ASSERT(player->getID() < 2, "Only player ids 0 and 1 supported");
	CAB_ASSERT(enemy->getID() < 2, "Only player ids 0 and 1 supported");
...
bool HLState::isNonWorkerCombatUnit(const BWAPI::UnitInterface *unit)
{
	...
	//{
	//	CAB_ASSERT(InformationManager::Instance().isCombatUnit(unit->getType())
...
bool HLState::isNonWorkerCombatUnit(const UnitInfo &unit)
{
	...
	//{
	//	CAB_ASSERT(InformationManager::Instance().isCombatUnit(unit.type)
...
void HLState::addSquads(const BWAPI::PlayerInterface *player)
{
	...
	for (auto &unit : player->getUnits()){
		if (isNonWorkerCombatUnit(unit))
		{
			...
			{
				Logger::LogAppendToFile(CAB_LOGFILE, "Exception while running BWTA::getRegion(%d, %d), ignoring this unit", unit->getPosition().x, unit->getPosition().y);
			}
		}
	}
	for (auto &it : units)
...
//
//		CAB_ASSERT(BOSS::Races::GetRace(_state[playerID].getRace()) == BWAPI::Races::Protoss, "Non protoss?");
//
//		//get goal units
...
//				}
//				else{
//					CAB_ASSERT(false, "Should have found a valid type here");
//				}
...
BOSS::BuildOrder HLState::getBuildOrder(const HLMove &move, int playerID) const
{
	...
	try{
		...
		Logger::LogAppendToFile(CAB_LOGFILE, "Hit choice point at beginning\n");
...
void HLState::applyAndForward(int depth, int frames, const std::array<HLMove, 2> &moves)
{
	...
	CAB_ASSERT(getRace(0) == BWAPI::Races::Protoss, "Non protoss?");
	CAB_ASSERT(getRace(1) == BWAPI::Races::Protoss, "Non protoss?");
	...
	for (int playerId = 0; playerId < 2; playerId++)
	{
		if (buildOrder[playerId].empty())
		{
			...
			//Logger::LogAppendToFile(CAB_LOGFILE, "Using build order planning, size: %d\n", buildOrder[playerId].size());
		}
	}
	...
	while (buildOrderIndex[0] < buildOrder[0].size() &&
		...
		//Logger::LogAppendToFile(CAB_LOGFILE, "Start new unit: %s to player %d frame %d\n", action.getName().c_str(), nextPlayer, _state[nextPlayer].getCurrentFrame());
		...
		{
			//Logger::LogAppendToFile(CAB_LOGFILE, "Hit choice point while forwarding 1\n");
		...
		CAB_ASSERT(_state[0].getMinerals() >= 0, "negative minerals: " + _state[0].getMinerals());
		CAB_ASSERT(_state[1].getMinerals() >= 0, "negative minerals: " + _state[1].getMinerals());
	}
	...
	//Logger::LogAppendToFile(CAB_LOGFILE, "Finished forwarding\n");
}
	...
	for (auto &unit : finishedUnits){
		if (unit.isUnit()){
			...
			{//let's add a new finished unit
				...
				//Logger::LogAppendToFile(CAB_LOGFILE, "Added new unit: %s to player %d frame %d\n", newUnit.type.getName().c_str(), playerID, _state[playerID].getCurrentFrame());
				if (unit.isWorker()){
				...
				//Logger::LogAppendToFile(CAB_LOGFILE, "Finished new unit: %s to player %d frame %d\n", unit.getName().c_str(), playerID, _state[playerID].getCurrentFrame());
...
void HLState::synchronizeNewUnits(int playerID, const std::vector<BOSS::ActionType> &finishedUnits, const 	BOSS::ActionType &startedUnit)
{
	if (startedUnit.isUnit()){
		...
		_unitData[playerID].addUnit(newUnit);
		//Logger::LogAppendToFile(CAB_LOGFILE, "Started new unit: %s to player %d frame %d\n", newUnit.type.getName().c_str(), playerID, _state[playerID].getCurrentFrame());
...
void HLState::forwardCombat(const std::array<std::vector<int>,2 > &squadIndex, int frames)
{
	...
	for (int p = 0; p < 2; p++)
	{
		CAB_ASSERT(!squadIndex[p].empty(), "Adding no squads!");
		for (size_t i = 0; i < squadIndex[p].size(); i++)
		{
			const HLSquad &squad = _squad[p][squadIndex[p][i]];
			CAB_ASSERT(!squad.empty(), "Adding empty squad!");
			...
			for (auto it = squad.begin(); it != squad.end();it++)
			{
					...
					&& SparCraft::System::isSupportedUnitType(unit.type))
				{
					...
					{
						e = 1;
						Logger::LogAppendToFile(CAB_LOGFILE,"Problem Adding Unit with ID: %d", unit.unitID);
					}
				}
			}
		}
	...
	CAB_ASSERT(state.numUnits(SparCraft::Players::Player_One) > 0 && state.numUnits(SparCraft::Players::Player_Two) > 0,
	...
	{
		Logger::LogAppendToFile(CAB_LOGFILE, state.toString());
		for (int p = 0; p < 2; p++)
		{
			Logger::LogAppendToFile(CAB_LOGFILE, "Player %d\n", p);
			for (size_t i = 0; i < squadIndex[p].size(); i++)
			{
				Logger::LogAppendToFile(CAB_LOGFILE, "Squad %d\n", i);
				const HLSquad &squad = _squad[p][squadIndex[p][i]];
				BWAPI::Position squadPos = squad.getCurrentRegion()->getCenter();
				...
				{
					const UnitInfo &unit = it->second;
					Logger::LogAppendToFile(CAB_LOGFILE, "	%s", unit.type.getName().c_str());
				}
			}
		}
	}
	...
	try
	{
		...
		g.play();
		CAB_ASSERT(prev!=g.getState().evalLTD2(SparCraft::Players::Player_One),
	...
	catch (int)
	{
		...
		CAB_ASSERT(false, "SparCraft FatalError, simulateCombat() threw");
	}
	...
	//Logger::LogAppendToFile(CAB_LOGFILE, "Combat executed for %d frames", frames);
}
...
std::vector<HLMove> HLState::getMoves(int playerID) const
{
	...
	//	}
	//	else if (enemy->getName().compare("CasiaBot") == 0)
	//	{
