...
void HLUnitData::addUnit(const UnitInfo &unit)
{
	...
	if (unit.type.isBuilding())
	{
		...
		{
			Logger::LogAppendToFile(CAB_LOGFILE, "Exception at BWTA::getRegion(%d, %d), ignoring unit", unit.lastPosition.x, unit.lastPosition.y);
		}
	}
}
...
bool HLUnitData::finishUnit(BWAPI::UnitType type)
{
	...
	CAB_ASSERT(found,"Couldn't find unit to finish: %s\n",type.getName().c_str());
	return found;
}
