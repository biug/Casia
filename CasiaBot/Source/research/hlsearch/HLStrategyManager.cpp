	...
	const std::unordered_map<short, short> &choices)
{
	if (race == BWAPI::Races::Protoss)
	{
		...
		case ProtossHighLevelSearch:
			...
			break;
		default:
			CAB_ASSERT(false,"Non existing Protoss strategy %d, using default", strategy);
		...
		CAB_ASSERT(false, "Unknown race when selecting strategy");
	...
	bool execution)
{
	...
	case 3:
		...
		break;
	default:
		CAB_ASSERT(false, "Wrong choice point option");
	}
