...
long long HLSearch::search(double timeLimit, int frameLimit, int maxHeight)
{
	...
	//{
	//	Logger::LogAppendToFile(CAB_LOGFILE, "Game seems decided, skipping HL search\n");
	//	return _stats.getRunningTimeMilliSecs();
	//}
	_minFrame = 1000000;
	Logger::LogAppendToFile(CAB_LOGFILE, _stats.header() + " score\tbest move\tmin frame\tmax frame\n"); 
	...
	for (int frames = 2000; frames <= frameLimit && !_timeUp; frames += 2000)
	{
		...
		Logger::LogAppendToFile(CAB_LOGFILE, _stats.toString());
		Logger::LogAppendToFile(CAB_LOGFILE, " %d %s %d %d\n", score, _bestMove.toString().c_str(), _minFrame - state.currentFrame(), _maxFrame - state.currentFrame());
...
int HLSearch::alphaBeta(const HLState& state, int depth, int height, int frameLimit, int turn, const HLMove &firstSideMove, int alpha, int beta)
{
	...
	CAB_ASSERT(moves.size() != 0, "No moves generated at depth %d", depth);
	if (!ttBest.isEmpty())
	{
		auto it=std::find(moves.begin(), moves.end(), ttBest);
		CAB_ASSERT(it != moves.end(), "Couldn't find best move among valid moves");
		if (it != moves.end())
		{
	...
	for (auto it = moves.begin(); it != moves.end() && !_timeUp;it++){
		auto m = *it;
		//Logger::LogAppendToFile(CAB_LOGFILE, "Searching depth %d, move %s, move %s\n", depth, firstSideMove.toString().c_str(), m.toString().c_str());
		...
		if (firstSideMove.isEmpty()){
			...
			//Logger::LogAppendToFile(CAB_LOGFILE, "Start frame: %d\n", newState.currentFrame());
			...
			//Logger::LogAppendToFile(CAB_LOGFILE, "Turn %d depth %d Checking moves: %s %s hash: %u\n", turn, depth,
			...
			try
			{
				...
				if (entry._hash == state.getHash(depth, movePair))//found
				{
					...
					//Logger::LogAppendToFile(CAB_LOGFILE, "Moves "+ firstSideMove.toString() + m.toString()+" matches\n");
					//Logger::LogAppendToFile(CAB_LOGFILE, " hashes: %u %u\n", entry._hash, entry._state.getHash());
					if (state.currentFrame() == entry._state.currentFrame()){//didn't progress
						//Logger::LogAppendToFile(CAB_LOGFILE, "No progress (cached)");
						continue;
					} 
				...
				else
				{
					...
					if (state.currentFrame() == newState.currentFrame()){//didn't progress
						//Logger::LogAppendToFile(CAB_LOGFILE, "No progress");
						continue;
					}
				...
				//Logger::LogAppendToFile(CAB_LOGFILE, "No Legal build order");
	...
	if (depth == 0){
		...
		else if (height == 2)//only use default move if first ID iteration
		{
			...
			Logger::LogAppendToFile(CAB_LOGFILE, "Couldn't find a strategy, using default\n");
		}
	}
